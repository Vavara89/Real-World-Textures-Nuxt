<template>
    <div class="detailModal" v-show="value">
        <div class="container">
            <div class="imageDetails">
                ...
                <div class="imageDetails-content">
                    <div class="previewImg" v-show="texture.image">
                        <img v-if="'image' in texture" :src="texture.image.url"/>
                    </div>
                    <div class="tutorial" v-show="texture.tutorial">
                ...
                <div class="imageDetails-footer">
                    <VueSlickCarousel ref="sliderNav" v-bind="navCarousel">
                        ...
                        <div class="imageItem">
                            <img v-if="'image' in texture" :src="texture.image.url" :alt="texture.image.alt" class />
                        </div>
                        <div class="imageItem">
                            <img v-if="'image' in texture" :src="texture.image.url" :alt="texture.image.alt" class />
                        </div>
            ...
            <div class="details">
                <div class="title">
                    <h4 class="name">{{ texture.title }}</h4>
                </div>
                <div class="credits-bookmark">
                    <div class="credits h3" v-if="texture.credits">
                    ...
                    <div class="option-item">
                        ...
                        <div class="option">
                            <ul class="consisting-list">
                                <li class="text" v-for="(item, index) in texture.consist" :key="'consist-' + index">{{ item }}</li>
                            </ul>
                        </div>
                    </div>
                </div>
                <div class="description">
                    ...
                    <p class="text">{{ texture.description }}</p>
                </div>
                <div class="brand">
                    <div class="label">
                        <label class="h3">Brand:</label>
                    </div>
                    <div class="option" v-if="texture.brand">
                        <label class="brand">{{ texture.brand.name }}</label>
                        <a class="brand-link" :href="texture.brand.link">{{ texture.brand.link }}</a>
                    </div>
                </div>
                <div class="download">
                    <button v-show="!processing" @click="process" class="button-primary">Download for {{ texture.credits }} credits</button>   
